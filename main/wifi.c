...
esp_callback_t __callback_failed = NULL;
...
static void event_handler(void* arg, esp_event_base_t event_base,int32_t event_id, void* event_data)
{
    ...
    if(event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP){
        ESP_LOGI(TAG,"evento obtuve ip\n");
        _state.wifi_got_ip = true;
        SECURE_CALL( __callback_connection);}

    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_CONNECTED){
        ESP_LOGI(TAG,"evento se conecto a la red wifi\n");
        _state.try_counter = 0;
...
esp_err_t wifi_desconnect(void){
    ...
    ESP_LOGI(TAG,"Desconectando wifi, liberando recursos\n");
